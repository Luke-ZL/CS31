...
{
	//  TRIVIAL:  Return the column the Flatulan is at.
...
void Flatulan::move()
{
	...
	{
	case 0:
		if ((m_row - 1 > 0) && ((m_row - 1 != m_city->player()->row()) || (m_col != m_city->player()->col()))) m_row -= 1;
		break;
	case 1:
		if ((m_row + 1 <= m_city->rows()) && ((m_row + 1 != m_city->player()->row()) || (m_col != m_city->player()->col()))) m_row += 1;
		break;
	case 2:
		if ((m_col - 1 > 0) && ((m_col - 1 != m_city->player()->col()) || (m_row != m_city->player()->row()))) m_col -= 1;
		break;
	case 3:
		if ((m_col + 1 <= m_city->cols()) && ((m_col + 1 != m_city->player()->col()) || (m_row != m_city->player()->row()))) m_col += 1;
		break;
	default:
		break;
	}
...
bool Flatulan::possiblyGetConverted()  // return true if converted
{
	...
	// Delete the following line and replace it with the correct code.
	return (randInt(0, 2) > 0);  // This implementation compiles, but is incorrect.
}
...
void Player::move(int dir)
{
	...
	{
	case 0:
		if (m_city->nFlatulansAt(m_row - 1, m_col) == 0) m_city->determineNewPosition(m_row, m_col, dir);
		break;
	case 1:
		if (m_city->nFlatulansAt(m_row + 1, m_col) == 0) m_city->determineNewPosition(m_row, m_col, dir);
		break;
	case 2:
		if (m_city->nFlatulansAt(m_row, m_col - 1) == 0) m_city->determineNewPosition(m_row, m_col, dir);
		break;
	case 3:
		if (m_city->nFlatulansAt(m_row, m_col + 1) == 0) m_city->determineNewPosition(m_row, m_col, dir);
		break;
...
void City::display() const
{
	...
	for (int i = 0; i < m_nFlatulans; i++)
	{
		if (grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] == '.') grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] = 'F';
		else if (grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] == 'F') grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] = '2';
		else if ((grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] >= '2') && (grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] <= '8')) grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1] += 1;
	}
...
void City::preachToFlatulansAroundPlayer()
{
	for (int i = 0; i < m_nFlatulans; i++)
	{
		if ((abs(m_flatulans[i]->row() - m_player->row()) <= 1) && (abs(m_flatulans[i]->col() - m_player->col()) <= 1) && m_flatulans[i]->possiblyGetConverted()) {
			for (int j = i; j < m_nFlatulans - 1; j++) *m_flatulans[j] = *m_flatulans[j + 1];
			i--;
...
void City::moveFlatulans()
{
	...
	{
		m_flatulans[k]->move();
		if (abs(m_flatulans[k]->row() - m_player->row()) + abs(m_flatulans[k]->col() - m_player->col()) == 1) m_player->getGassed();
